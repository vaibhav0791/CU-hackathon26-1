...
const AnalysisPage = () => {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      pdf.text('PHARMA-AI — FORMULATION ANALYSIS REPORT', margin, 14);
      pdf.setFontSize(8);
      pdf.setTextColor(148, 163, 184);
      pdf.text(`Generated: ${new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}`, margin, 22);
      pdf.text(`Analysis ID: ${result.id || 'N/A'}${result.is_experimental ? '  |  EXPERIMENTAL COMPOUND' : ''}`, margin, 29);
      y = 48;
      ...
      pdf.text(`SMILES: ${(result.smiles || '').substring(0, 65)}${(result.smiles || '').length > 65 ? '...' : ''}`, margin + 6, y + 19);
      pdf.text(`MW: ${result.molecular_weight} g/mol  |  Dose: ${result.dose_mg} mg  |  Class: ${result.drug_info?.therapeutic_class || 'N/A'}  |  BCS: ${result.drug_info?.bcs_class || 'N/A'}`, margin + 6, y + 27);
      y += 40;

      const addSection = (title, rgbColor, fn) => {
        if (y > pageHeight - 30) { pdf.addPage(); y = margin; }
        pdf.setFontSize(11); pdf.setTextColor(...rgbColor); pdf.setFont('helvetica', 'bold');
        pdf.text(title, margin, y);
        pdf.setDrawColor(...rgbColor); pdf.setLineWidth(0.4);
        pdf.line(margin, y + 2, pageWidth - margin, y + 2);
        y += 8; pdf.setFont('helvetica', 'normal'); pdf.setFontSize(9); pdf.setTextColor(203, 213, 225);
        fn(); y += 5;
      };

      const addLine = (text, indent = 0) => {
        if (y > pageHeight - 14) { pdf.addPage(); y = margin; }
        const lines = pdf.splitTextToSize(text, pageWidth - margin * 2 - indent);
        pdf.text(lines, margin + indent, y);
        y += lines.length * 4.8;
      };

      const addNLSummary = (text, color) => {
        if (!text) return;
        if (y > pageHeight - 24) { pdf.addPage(); y = margin; }
        pdf.setFillColor(...color, 0.08);
        const lines = pdf.splitTextToSize(`“${text}”`, pageWidth - margin * 2 - 8);
        const boxH = lines.length * 5 + 10;
        pdf.setFillColor(color[0], color[1], color[2]);
        pdf.setAlpha ? null : null; // jsPDF basic
        pdf.roundedRect(margin, y, pageWidth - margin * 2, boxH, 2, 2, 'S');
        pdf.setFontSize(8); pdf.setTextColor(200, 220, 240); pdf.setFont('helvetica', 'italic');
      ...
      // Molecule Overview
      if (result.molecule_overview) {
        addSection('MOLECULE OVERVIEW', [0, 242, 255], () => {
          addLine(`Inferred Class: ${result.molecule_overview.inferred_class}`);
          addLine(`Drug-Likeness: ${result.molecule_overview.drug_likeness}`);
          if (result.molecule_overview.key_features?.length) {
            addLine('Key Structural Features:');
            result.molecule_overview.key_features.forEach(f => addLine(`• ${f}`, 4));
          }
        });
      }
      ...
      addSection('1. SOLUBILITY PREDICTION', [0, 242, 255], () => {
        if (result.solubility) {
          addLine(`Classification: ${result.solubility.classification}  |  Score: ${result.solubility.prediction}  |  Accuracy: ${result.solubility.accuracy}%`);
          ...
          addNLSummary(result.solubility.natural_language_summary, [0, 200, 220]);
        }
      });
        ...
        if (result.excipients) {
          ...
          ['binders', 'fillers', 'disintegrants', 'lubricants'].forEach(cat => {
            if (result.excipients[cat]?.length) {
              ...
              result.excipients[cat].forEach(e => addLine(`• ${e.name} (${e.grade || ''}) — ${e.recommended_conc}`, 4));
            }
          });
          result.excipients.incompatibilities?.forEach(i => addLine(`⚠ ${i}`, 4));
          addNLSummary(result.excipients.natural_language_summary, [140, 60, 255]);
        }
      });

      // Stability
      addSection('3. STABILITY FORECAST', [0, 200, 130], () => {
        if (result.stability) {
          addLine(`Predicted Shelf Life: ${result.stability.shelf_life_years} years  |  Primary Degradation: ${result.stability.primary_degradation}`);
          const sc = result.stability.storage_conditions;
          if (sc) addLine(`Storage: ${sc.temperature}  |  ${sc.humidity}  |  Container: ${sc.container}`);
          result.stability.degradation_mechanisms?.forEach(m => addLine(`• ${m}`, 4));
          addNLSummary(result.stability.natural_language_summary, [0, 180, 120]);
        }
      });

      // PK
      addSection('4. PK-COMPATIBILITY & BIOAVAILABILITY', [220, 150, 20], () => {
        if (result.pk_compatibility) {
          const pk = result.pk_compatibility;
          addLine(`Bioavailability: ${pk.bioavailability_percent}%  |  Tmax: ${pk.tmax_hours}h  |  T½: ${pk.t_half_hours}h  |  Protein Binding: ${pk.protein_binding_percent}%`);
          if (pk.metabolism) addLine(`Metabolism: ${pk.metabolism.primary_enzyme}  |  1st Pass: ${pk.metabolism.first_pass}`);
          addLine(`Recommended: ${pk.recommended_dosage_form}  |  Frequency: ${pk.dosing_frequency}`);
          addNLSummary(pk.natural_language_summary, [220, 160, 40]);
        }
      });
      ...
      for (let i = 1; i <= totalPages; i++) {
        pdf.setPage(i);
        pdf.setFontSize(7); pdf.setTextColor(71, 85, 105); pdf.setFont('helvetica', 'normal');
        pdf.text(`PHARMA-AI Formulation Optimizer  |  Page ${i} of ${totalPages}`, margin, pageHeight - 6);
        pdf.text('For research use only. Consult a qualified pharmaceutical scientist before manufacturing.', pageWidth - margin, pageHeight - 6, { align: 'right' });
      }
