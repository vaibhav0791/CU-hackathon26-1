# backend/extract_cancer_data.py
import asyncio
import json
import httpx
...
async def run_cancer_extraction():
    print("⚡ CDO Data Engine: Initializing TARGETED EGFR Cancer Pathway Extraction...")
    audit_engine = PharmaAIDiscoveryAudit()
    
    # Target CHEMBL203 (Human EGFR), a major master driver in oncology/cancer studies
    url = "https://www.ebi.ac.uk/chembl/api/data/activity.json?target_chembl_id=CHEMBL203&limit=100&format=json"
    
    print("📡 Requesting direct bioactivities for oncology target EGFR (CHEMBL203)...")
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        try:
            ...
            print(f"📥 Successfully received {len(activities)} raw cancer assays from ChEMBL.")
                ...
                if smiles and chembl_id:
                    ...
                    if uid not in seen_in_this_batch:
                        ...
                        pristine_cancer_records.append({
                            ...
                            "smiles": smiles,
                            "chembl_id": chembl_id,
                            "pubchem_cid": "Ingested via Oncology Target Gateway",
                            "molecular_weight": mw,
                            "log_p": logp,
                            "assay_context": act.get("assay_description", "EGFR Cancer Receptor Inhibition Assay"),
                            "ingestion_timestamp": "2026-06-07T00:50:00Z"
                        })
            ...
            print("\n" + "="*60)
            print("🏁 CANCER TARGETED EXTRACTION COMPLETE!")
            print(f"📈 Total Clean Cancer Records Found: {len(pristine_cancer_records)}")
